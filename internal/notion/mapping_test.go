...
func TestPushPayloadFromIssueOmitsLabelsForLiveMCPCompatibility(t *testing.T) {
	t.Parallel()
	...
	if payload.Issues[0].Labels != nil {
		t.Fatalf("labels = %v, want nil for live MCP compatibility", payload.Issues[0].Labels)
	}
}
